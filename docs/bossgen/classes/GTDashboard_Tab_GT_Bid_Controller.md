---
layout: default
title: GTDashboard_Tab_GT_Bid_Controller
parent: classes
---
# Metadata Type
classes
...
# Filename 
GTDashboard_Tab_GT_Bid_Controller


# Raw XML
```
global class GTDashboard_Tab_GT_Bid_Controller {
    ...
    public GTDashboard gtDashboard {get;set{bidDataMap = value.bidDataMap;gtDashboard = value;}}
    ...
    public GTDashboard_Tab_GT_Bid_Controller() {
        bidData = new Bid__c();
        System.debug('GTDashboard_Tab_GT_Bid_Controller.tripBidNumber: ' + tripBidNumber);
    ...
    public void Init(){
        ...
        production_markettypes.addAll(GTDashboardController.PickThePicklist('Opportunity','Market_Type__c'));
        groundFilter = new Ground__c();
        List<wrapperSearchPL> consValueList = new List<wrapperSearchPL>();
        Map<Object,List<String>> dependValuesByControlValue = GTDashboardController.getDependentPicklistValues(Concessions__c.Concessions_Requested__c);
        ...
        for(SelectOption so : GTDashboardController.PickThePicklist('Ground__c','Airline_Info_Special_Notes__c')){
            consValueList.add(new wrapperSearchPL(so.getValue(),so.getValue()));
        }
        airlineInfoJson = JSON.serialize(consValueList);
        
        consValueList = new List<wrapperSearchPL>();
        for(SelectOption so : GTDashboardController.PickThePicklist('Ground__c','Group_Type__c')){
            consValueList.add(new wrapperSearchPL(so.getValue(),so.getValue()));
        }
        grouptypeJson = JSON.serialize(consValueList);    
        
        consValueList = new List<wrapperSearchPL>();
        for(SelectOption so : GTDashboardController.PickThePicklist('Ground__c','Model__c')){
            consValueList.add(new wrapperSearchPL(so.getValue(),so.getValue()));
        }
        modelJson = JSON.serialize(consValueList);
        
        consValueList = new List<wrapperSearchPL>();
        for(SelectOption so : GTDashboardController.PickThePicklist('Ground__c','Make__c')){
            consValueList.add(new wrapperSearchPL(so.getValue(),so.getValue()));
        }
        makeJson = JSON.serialize(consValueList);
    ...
    public void uncontractBid(){
        ...
        if(bidDataMap.containsKey(tripBidNumber) && bidDataMap.get(tripBidNumber).b.Status_Previous__c != null && bidDataMap.get(tripBidNumber).b.Status_Previous__c != ''){
            ...
            if(bidDataMap.get(tripBidNumber).b.GT__c != null){
                //update new Ground__c(Id=bidDataMap.get(tripBidNumber).b.GT__c,Status_Ground__c=bidDataMap.get(tripBidNumber).b.GT__r.Status_Previous__c);
                update new Ground__c(Id=bidDataMap.get(tripBidNumber).b.GT__c,Status_Ground__c='Bid Request Stage');
                List<Itinerary__c> itineraryAuxList =  [SELECT Id FROM Itinerary__c WHERE GT__c =: bidDataMap.get(tripBidNumber).b.GT__c AND RecordType.DeveloperName = 'GT_Itinerary'];
                //if(itineraryAuxList.size() > 0) update new Itinerary__c(Id = itineraryAuxList[0].Id,Status__c = bidDataMap.get(tripBidNumber).b.GT__r.Status_Previous__c);
                if(itineraryAuxList.size() > 0) update new Itinerary__c(Id = itineraryAuxList[0].Id,Status__c = 'Bid Request Stage');
            }
            
            /*List<Ground__c> subTripList = [SELECT Id FROM Ground__c WHERE Bid_Sub_Trip__c =: bidDataMap.get(tripBidNumber).b.Id AND RecordType.DeveloperName = 'GT_Sub_Trip_Details'];
            delete [SELECT Id FROM Ground__c WHERE GT_Sub_Trip_Details__c IN: subTripList AND RecordType.DeveloperName = 'GT_Travel_Details']; //travels bid
            delete [SELECT Id FROM Concessions__c WHERE Bid__c =: bidDataMap.get(tripBidNumber).b.Id]; //additional fees
            delete [SELECT Id FROM Ground__c WHERE Bid__c =: bidDataMap.get(tripBidNumber).b.Id AND RecordType.DeveloperName = 'Vehicle_Type']; //vehicles bid
            ...
            delete subTripList; //subtrips bid*/
        }
    ...
    public void contractBid(){
        ...
        if(tripHaveContracted){
            if(bidDataMap.get(tripBidNumber).b.GT__c != null){
                Ground__c f = [SELECT Id, RecordTypeId, Start_Date__c, End_Date__c, Start_City__c, Description__c, GT_Preferences__c, Status_Ground__c, Trace_Date__c, Deadline_Date__c,
                                Group_Type__c, Service_Type__c FROM Ground__c WHERE Id =: bidDataMap.get(tripBidNumber).b.GT__c];
                Itinerary__c it = [SELECT Id, Production__c, Status__c, Start_Date__c, End_Date__c, GT__c, City__c FROM Itinerary__c WHERE GT__c =: bidDataMap.get(tripBidNumber).b.GT__c];
                Ground__c newGround = f.clone();
                newGround.Vendor__c = bidDataMap.get(tripBidNumber).b.Vendor__c;
                newGround.Status_Previous__c = bidDataMap.get(tripBidNumber).b.GT__r.Status_Ground__c;
                newGround.Status_Ground__c = 'Contracted';
                newGround.Production_Ground__c = bidDataMap.get(tripBidNumber).b.Production__c;
                newGround.Vendor_lookup__c = bidData.Vendor__c;
                system.debug(newGround);
                insert newGround;
                
                cloneInformationToFreight(bidDataMap.get(tripBidNumber).b,newGround,ground_vehicletypeRT,ground_GTSubStripDetailsRT,ground_GTTravelDetailsRT);
                
                bidDataMap.get(tripBidNumber).b.GT__c = newGround.Id;
                bidData.GT__c = newGround.Id;
                update bidData;
                ...
                newItinerary.GT__c = newGround.Id;
                system.debug(newItinerary);
                insert newItinerary;
                
                //cloneInformationToBid(bidDataMap.get(tripBidNumber).b,ground_vehicletypeRT,ground_GTSubStripDetailsRT,ground_GTTravelDetailsRT);
            ...
            if(bidDataMap.get(tripBidNumber).b.GT__c != null){
                update new Ground__c(Id=bidDataMap.get(tripBidNumber).b.GT__c,Vendor__c=bidDataMap.get(tripBidNumber).b.Vendor__c,Status_Previous__c=bidDataMap.get(tripBidNumber).b.GT__r.Status_Ground__c,Status_Ground__c='Contracted');
                List<Itinerary__c> itineraryAuxList =  [SELECT Id FROM Itinerary__c WHERE GT__c =: bidDataMap.get(tripBidNumber).b.GT__c AND RecordType.DeveloperName = 'GT_Itinerary'];
    ...
    public void sendResendBid(){
        if(bidData != null && contactPrimarySelected != null){
            ...
            User userActual = GTDashboardController.verifyEmailUser(UserInfo.getUserId(),opportunityid,bidData.GT__c);
            ...
            Ground__c gAux = new Ground__c(Id=bidData.GT__c);
            
            //New Journals
            journalAux = new Journal__c(
                RecordTypeId = journal_bidjournalRT,
                Bid__c = bidData.Id,
                Journal_Entry__c = 'Ground Travel Bid Request Link has been sent to ' + contactMap.get(contactPrimarySelected).Name + ' at ' + contactMap.get(contactPrimarySelected).Email
            );
            ...
            if(bidData.Vendor__c != null){
                for(ContactWrapper cw : contactsByVendorBidMap.get(bidData.Vendor__c)) if(!cw.disabled && cw.selected) ccAddresses.add(cw.c.Email);
            }
            if(bidData.Vendor__r.ParentId != null){
                for(ContactWrapper cw : contactsByVendorParentBidMap.get(bidData.Vendor__r.ParentId)) if(!cw.disabled && cw.selected) ccAddresses.add(cw.c.Email);
            }
            ...
            subject = (templateLink.size() > 0 ? templateLink[0].Subject.replace('{ProductionName}',bidData.Production__r.Name)
                       .replace('{StartDate}',(bidData.GT__r.Start_Date__c != null ? Datetime.newInstance(bidData.GT__r.Start_Date__c.year(),bidData.GT__r.Start_Date__c.month(),bidData.GT__r.Start_Date__c.day()).format('dd MMMMM yyyy') : ''))
                       .replace('{EndDate}',(bidData.GT__r.End_Date__c != null ? Datetime.newInstance(bidData.GT__r.End_Date__c.year(),bidData.GT__r.End_Date__c.month(),bidData.GT__r.End_Date__c.day()).format('dd MMMMM yyyy') : ''))
                       .replace('{BidNumber}',bidData.Name)
                       : '');
            body = 'Dear ' + contactMap.get(contactPrimarySelected).Name + ',<br/><br/>' +  
                (templateLink.size() > 0 ? templateLink[0].HtmlValue.replace('{GroundTravelCoordinator}',userActual != null && userActual.Name != null ? userActual.Name : '')
                 ...
                 .replace('{BidURL}','<a href="' + (communityURLvalue != null ? communityURLvalue : '') + '/s/?service=ground&record=' + bidData.Id + '" target="_blank">' + (communityURLvalue != null ? communityURLvalue : '') + '/s/?service=ground&record=' + bidData.Id + '</a>')
            ...
            if(bidData.GT__r.Status_Ground__c != 'Contracted' || bidData.GT__r.Status_Ground__c != 'In Contracting'){
                gAux.Status_Ground__c = 'Bid Request Stage';
                List<Itinerary__c> itineraries =  [SELECT Id FROM Itinerary__c WHERE GT__c =:bidData.GT__c AND RecordType.DeveloperName = 'GT_Itinerary'];
    ...
    public void addVehicleBid(){
		if(bidData != null && bidData.id != null){
            ...
            newVehicleBid.RecordTypeId = ground_vehicletypeRT;
            newVehicleBid.Production_Ground__c = opportunityId;
            newVehicleBid.Amenities__c = (vehicle_amenities != null && vehicle_amenities != '' ? vehicle_amenities.replace(',',';') : null);
            //if(bidData.Status__c == 'Contracted') newVehicleBid.Bid__c = bidData.Id ; else newVehicleBid.GT_Trip_Details__c = bidData.GT__c;
            newVehicleBid.Bid__c = bidData.Id;
            newVehicleBid.Make__c = vehicle_make;
            newVehicleBid.Model__c = vehicle_model;
    ...
    public void getVehiclesByBid(){
        vehiclesByBidList = new List<Ground__c>();
        if(bidData != null && bidData.Id != null){
            //if(bidData.Status__c == 'Contracted'){
                vehiclesByBidList = [SELECT Id, Production_Ground__c, RecordTypeId, RecordType.Name, Vehicle_Ref__c, Vehicle_Type__c, Capacity__c, Make__c, Model__c, Year__c, Amenities__c, Other_Amenities__c, Gear_Luggage_Space__c
                                     FROM Ground__c WHERE RecordType.Name = 'Vehicle Type' AND Production_Ground__c =: opportunityId 
                                     AND Bid__c =: bidData.Id AND Bid_Sub_Trip__c = NULL AND GT_Sub_Trip_Details__c = NULL AND Sales_GT_Details__c = NULL];
            /*}else{
                vehiclesByBidList = [SELECT Id, Production_Ground__c, RecordTypeId, RecordType.Name, Vehicle_Ref__c, Vehicle_Type__c, Capacity__c, Make__c, Model__c, Year__c, Amenities__c, Other_Amenities__c 
                                     FROM Ground__c WHERE RecordType.Name = 'Vehicle Type' AND Production_Ground__c =: opportunityId AND GT_Trip_Details__c =: bidData.GT__c
                                     AND Bid__c = NULL AND Bid_Sub_Trip__c = NULL AND GT_Sub_Trip_Details__c = NULL AND Sales_GT_Details__c = NULL];
            }*/
        }
    }
    public void saveTravelBid(){
        ...
        if(String.isEmpty(travelActualId)) travel.Production_Ground__c = opportunityId;
        ...
        if(newtravel_updatemaster){
            ...
            if(newtravel_startdate != null){
                gAux.Start_Date__c = bidData.GT__r.Start_Date__c != null ? (bidData.GT__r.Start_Date__c <= newtravel_startdate ? bidData.GT__r.Start_Date__c : newtravel_startdate) : newtravel_startdate;
            }
            if(newtravel_enddate != null){
                gAux.End_Date__c = bidData.GT__r.End_Date__c != null ? (bidData.GT__r.End_Date__c >= newtravel_enddate ? bidData.GT__r.End_Date__c : newtravel_enddate) : newtravel_enddate;
            }
            update gAux;
        }
        getSubStripByBid();
    ...
    public void getSubStripByBid(){
        ...
        //if(bidData.Status__c == 'Contracted'){
        for(Ground__c subTrip:[Select id, GT_Freight__c, Trip_Order__c, Sub_Trip_Type__c, Description__c, Sub_Trip_Notes__c, Location_Details_Drop_Off__c, 
                               (Select id, Vehicle_Lookup__c, Line__c, Vehicle_Ref__c, Start_Date__c, Start_Date_Time__c, End_Date__c, End_Date_Time__c, Travel_Type__c, Group_Type__c, Location_Type__c, Location_Pick_up__c, 
                                Location_Drop_off__c, Location_Details__c, Airline_Info_Special_Notes__c, Start_Date_Full__c, End_Date_Full__c, Start_Date_Time_Text__c, End_Date_Time_Text__c, Location_Drop_Off_Type__c, Location_Details_Drop_Off__c
                                from GTTravels__r order by CreatedDate ASC)     //order by Line__c  -- Changes as per 176568992
                               from Ground__c where Bid_Sub_Trip__c =:bidDataMap.get(tripBidNumber).b.Id and RecordType.DeveloperName = 'GT_Sub_Trip_Details' order by CreatedDate ASC]){        //order by Trip_Order__c, Line__c -- Changes as per 176568992
                                   if(subTrip.GTTravels__r != null){
                                       for(Ground__c travel : subTrip.GTTravels__r){
        ...
        /*}else{
            for(Ground__c subTrip:[Select id, GT_Freight__c, Trip_Order__c, Sub_Trip_Type__c, Description__c, Sub_Trip_Notes__c, (Select id, Line__c, Vehicle_Ref__c, Start_Date__c, Start_Date_Time__c, End_Date__c, End_Date_Time__c, Travel_Type__c, Group_Type__c, Location_Type__c, Location_Pick_up__c, Location_Drop_off__c, Location_Details__c, Airline_Info_Special_Notes__c, Start_Date_Full__c, End_Date_Full__c from GTTravels__r order by Line__c) 
                                   from Ground__c where GT_Trip_Details__c =:bidDataMap.get(tripBidNumber).b.GT__c and RecordType.DeveloperName = 'GT_Sub_Trip_Details' order by Trip_Order__c, Line__c]){
                                       bidSubTrips.add(subTrip);
                                       hasFreightMap.put(subtrip.Id,subtrip.GT_Freight__c != null ? true : false);
                                       bidSubTripTravelsMap.put(subTrip.id, subTrip.GTTravels__r);
                                   }
    ...
    public void saveEditConcessionBid(){
        if(concessionActual != null && concessionActual != ''){
            ...
            for(Concessions__c c : [SELECT Id, Concessions_Requested__c, Concession_Type__c, Notes__c, Included_Excluded__c, Rate__c, Order__c FROM Concessions__c WHERE Production__c =: opportunityId AND Bid__c =:bidData.Id and Concession_Type__c = 'GT' ORDER BY Order__c ASC NULLS LAST, CreatedDate DESC]){
                if(c.Order__c != null){
    ...
    public void addConcessionBid(){
        if(bidData != null && bidData.Id != null){
            ...
            newConcessionBid.Concession_Type__c = 'GT';
    ...
    public void getConcessionsByBid(){
        ...
        if(bidData != null && bidData.Id != null){
                concessionsRequestedBid = [SELECT Id, Concessions_Requested__c, Concession_Type__c, Notes__c, Included_Excluded__c, Rate__c, Order__c
                                           FROM Concessions__c 
                                           WHERE Production__c =: opportunityId AND Bid__c =:bidData.Id and Concession_Type__c = 'GT' ORDER BY Order__c ASC NULLS LAST, CreatedDate DESC];
        }
    }
    public void searchBidHistory(){
        String vendor = bidData.Vendor__c;
        bidRelatedVendorList = new List<BidVendorWrapper>();
        String query = 'SELECT Id, Name, Production__c, Production__r.Name, GT__c, GT__r.Start_Date__c, GT__r.End_Date__c, GT_Description__c,' +
        ...
        if(bidhistorysearch_onlyirstays != null) query += ' AND GT__r.RecordType.Name = \'Individual Reservation\'';
        ...
        if(bidhistorysearch_from != null && String.valueOf(bidhistorysearch_from).trim() != '') query += ' AND GT__r.Start_Date__c >=: bidhistorysearch_from';
        if(bidhistorysearch_to != null && String.valueOf(bidhistorysearch_to).trim() != '') query += ' AND GT__r.End_Date__c <=: bidhistorysearch_to';
        
        if(orderBidHistoryOrientation == null) orderBidHistoryOrientation = 'ASC';
        if(orderCheckBidHistory){
            if(orderChangeBidHistory) orderBidHistoryOrientation = 'ASC';
            else{
                if(orderBidHistoryOrientation == 'ASC') orderBidHistoryOrientation = 'DESC'; else orderBidHistoryOrientation = 'ASC';
            }
        }
        for(Bid__c b : Database.query(query)) bidRelatedVendorList.add(new BidVendorWrapper(b,b.Name,b.Production__r.Name,b.GT__r.Start_Date__c,b.GT__r.End_Date__c,b.GT_Description__c,null,b.Status__c,b.No_Bid_reason__c,b.Venue__r.Name,b.Vendor__r.Parent.Name,b.CreatedDate.date()));
        bidRelatedVendorList.sort();
    }
    public void openBidHistory(){
        bidRelatedVendorList = new List<BidVendorWrapper>();
        Set<String> bidIdsAux = new Set<String>();
        for(Bid__c b : [SELECT Id, Name, Production__c, Production__r.Name, GT__c, GT__r.Start_Date__c, GT__r.End_Date__c, GT_Description__c,
                        Status__c, No_Bid_reason__c, Venue__c, Venue__r.Name, Vendor__r.ParentId, Vendor__r.Parent.Name, CreatedDate
                        FROM Bid__c WHERE Vendor__c =:bidData.Vendor__c AND RecordType.Name = 'GT Bid' ORDER BY CreatedDate DESC]){
            bidIdsAux.add(b.Id);
            bidRelatedVendorList.add(new BidVendorWrapper(b,b.Name,b.Production__r.Name,b.GT__r.Start_Date__c,b.GT__r.End_Date__c,b.GT_Description__c,null,b.Status__c,b.No_Bid_reason__c,b.Venue__r.Name,b.Vendor__r.Parent.Name,b.CreatedDate.date()));
        }
        rateBidMap = new Map<String,Revenue__c>();
        ...
        for(String bididaux : bidIdsAux) if(rateBidMap.get(bididaux) == null) rateBidMap.put(bididaux, new Revenue__c());
        for(BidVendorWrapper bw : bidRelatedVendorList) bw.rate = 0;
    }
    public void getBidInformation(){
        tripHaveContracted = false;
        if(tripBidNumber != null && bidDataMap.get(tripBidNumber) != null){
            bidData = [Select Id, RecordTypeId, RecordType.Name, Name, Production__c, Production__r.Name, Production__r.Production_ID__c, Production__r.Account.Name, Production__r.Account.Physical_Address__c, Production__r.Office_Location__c, GT__c, GT__r.Name, GT__r.Start_Date__c, GT__r.End_Date__c, GT__r.Status_Ground__c, Vendor__c, Vendor__r.Vendor_ID__c, Vendor__r.Phone, 
                       ...
                       Include_in_options__c, Options__c, Include_in_option_cover_sheet__c, No_Bid_reason__c, Trips_Details_Update_Contact_Id__c, Trip_Details_Update_Terms__c, Trip_Details_Update_Payment_Policy__c, Trip_Details_Update_Cancel_Policy__c, 
                       Trip_Details_Update_Last_Modified_By__c, Trip_Details_Update_Last_Send_By__c, Trip_Details_Update_Last_Send_Date__c, Rate_Agreement_Contact_Id__c, Rate_Agreement_Production_Contact_Id__c, Rate_Agreement_Terms__c, Rate_Agreement_Payment_Policy__c, 
                       Rate_Agreement_Cancellation_Policy__c, Rate_Agreement_Last_Send_By__c, Rate_Agreement_Last_Send_Date__c, Transportation_Contract_Contact_Id__c, Transportation_Contract_Terms__c, Transportation_Contract_Payment_Policy__c, Transportation_Contract_Cancel_Policy__c,  
                       Transportation_Contract_Last_Send_By__c, Transportation_Contract_Last_Send_Date__c, Billing_Options__c, Form_Rate_Agreement__c, Form_Transportation_Contract__c, Form_Trip_Details_Update__c, GT__r.GT_Preferences__c
            ...
            if(vfpConfiguration != null){
                ...
                vfpConfiguration.parentId = bidData.GT__c;
                vfpConfiguration.productionId = opportunityId;
                vfpConfiguration.parentSObject = 'Ground';
            }
            ...
            */
            User emailFromUser = GTDashboardController.verifyEmailUser(UserInfo.getUserId(), opportunityid,bidData.GT__c);
            ...
            for(APXTConga4__Conga_Merge_Query__c congaQuery:[Select Id, APXTConga4__Name__c From APXTConga4__Conga_Merge_Query__c Where APXTConga4__Name__c in ('GT - BidsWithOptionsByBidId', 'GT - SubTripsByBidId', 'GT - BidById', 'GT - ContactById', 'GT - ProductionAssociationsByOppId', 'GT - UserById')]) congaQueriesMap.put(congaQuery.APXTConga4__Name__c, congaQuery);
            ...
            for(APXTConga4__Conga_Template__c congaTemplate:[Select Id, APXTConga4__Name__c From APXTConga4__Conga_Template__c Where APXTConga4__Name__c in ('GT - Ground Travel Bid', 'GT - Ground Travel Options')]) congaTemplatesMap.put(congaTemplate.APXTConga4__Name__c, congaTemplate);
            previewBidOptions_congaTemplateId = congaTemplatesMap.get('GT - Ground Travel Options') != null ? congaTemplatesMap.get('GT - Ground Travel Options').id : null;
            resendBidAgreement_congaTemplateId = congaTemplatesMap.get('GT - Ground Travel Bid') != null ? congaTemplatesMap.get('GT - Ground Travel Bid').id : null;
            /**
            *  @/Conga
            */
            ...
            coordinatorGT = null;
            for(Production_Associations__c pa : [SELECT Id, Production_Opp__c, Contact__c, Contact__r.Name, Contact__r.Title, Contact__r.Order__c, Contact__r.Email, Contact__r.Phone, Contact__r.MobilePhone, User__c, Team_Roles__c, User__r.Name, Role__c FROM Production_Associations__c WHERE Production_Opp__c =:opportunityId ORDER BY Contact__r.Name ASC]){
                if(pa.User__c != null && pa.Team_Roles__c != null){
                    if(pa.Team_Roles__c.contains('Primary GT Coordinator')) coordinatorGT = pa.User__c; 
                }
            }
        }
    }
    ...
    global static String searchVehicleTrip(String query, String bidid){
        ...
        String description;
        //Bid__c b = [SELECT Id, Status__c, GT__c FROM Bid__c WHERE Id =: bidid];
        List<Ground__c> searchList;
        
        searchList = [SELECT Id, Vehicle_Ref__c FROM Ground__c WHERE Vehicle_Ref__c LIKE: query AND Bid__c =: bidid AND RecordType.Name = 'Vehicle Type' AND GT_Trip_Details__c = NULL AND Bid_Sub_Trip__c = NULL AND GT_Sub_Trip_Details__c = NULL AND Sales_GT_Details__c = NULL];

        for(Ground__c g : searchList){
    ...
    public void openBidAgreement(){
        ...
        if((bidAgreement_type == 'Rate Agreement' && String.isNotBlank(bid.Form_Rate_Agreement__c)) || (bidAgreement_type == 'Trip Details Update' && String.isNotBlank(bid.Form_Trip_Details_Update__c)) || (bidAgreement_type == 'Transportation Contract' && String.isNotBlank(bid.Form_Transportation_Contract__c))) draft = [SELECT Id, Name, Salutation__c, Attention__c, Primary_Contact_Id__c, Production_Assoc_Contact_Id__c, CC__c, Subject__C, Date__c, BCC__c, Body__c, Terms__c, CreatedBy.Name, CreatedDate, LastModifiedBy.Name, LastModifiedDate, Payment_Policy__c, Cancellation_Policy__c, Credit_Card_Id__c FROM Draft__c WHERE Name =:(bidAgreement_type == 'Rate Agreement' ? bid.Form_Rate_Agreement__c : bidAgreement_type == 'Trip Details Update' ? bid.Form_Trip_Details_Update__c : bidAgreement_type == 'Transportation Contract' ? bid.Form_Transportation_Contract__c : '')];            
            ...
            draft.Terms__c = bidAgreement_type == 'Rate Agreement' ? (bidData.Rate_Agreement_Terms__c != null && bidData.Rate_Agreement_Terms__c.trim() != '' ? bidData.Rate_Agreement_Terms__c : (!saleGround.isEmpty() ? saleGround[0].Contracting_Terms__c : '')) : bidAgreement_type == 'Trip Details Update' ? bidData.Trip_Details_Update_Terms__c : bidAgreement_type == 'Transportation Contract' ? (!saleGround.isEmpty() ? saleGround[0].Contracting_Terms__c : '<ul><li>Driver(s) must assist with luggage.</li><li>Driver(s) will be on time, drive safely and only use cell phones in emergencies.</li><li>Driver(s) name and cell will be provided prior to travel.</li><li>Driver(s) are responsible for having directions to destination.</li><li>Bus company will not sub-contract coaches.</li><li>Any mechanical failure, or driver/dispatch negligence can cause major issues for their itinerary. This includes being lost or stopping unnecessarily. Bus company will make all efforts to fix any mechanical issues immediately. All drivers should be aware that any requests from Tour Management should be adhered to. Weather or roadway conditions, infringement on D.O.T regulations or the intervention of any governmental body are of exception. Road Rebel should be contacted immediately should any of the events listed occur.</li></ul>') : '';
            ...
            insert draft;
            draft = [SELECT Id, Salutation__c, Name, Attention__c, Primary_Contact_Id__c, Production_Assoc_Contact_Id__c, CC__c, Subject__C, Date__c, BCC__c, Body__c, Terms__c, CreatedBy.Name, CreatedDate, LastModifiedBy.Name, LastModifiedDate, Payment_Policy__c, Cancellation_Policy__c, Credit_Card_Id__c FROM Draft__c WHERE Id = :draft.Id];
        ...
        bidAgreementConcessions = [Select id, Concessions_Requested__c, Concession_Provided__c, Concession_Type__c, Notes__c, Included_Excluded__c, Rate__c from Concessions__c where Bid__c =:bidData.Id and Concession_Type__c = 'GT'];
        ...
        */
        ...
        congaQueriesMap = new Map<String, APXTConga4__Conga_Merge_Query__c>();
        for(APXTConga4__Conga_Merge_Query__c congaQuery:[Select Id, APXTConga4__Name__c From APXTConga4__Conga_Merge_Query__c Where APXTConga4__Name__c in ('GT - BidById', 'GT - SubTripsByBidId', 'GT - ContactById', 'GT - ClientContactById', 'GT - ProductionAssociationsByOppId', 'GT - UserById')]) congaQueriesMap.put(congaQuery.APXTConga4__Name__c, congaQuery);
    ...
    public List<SelectOption> getBidAgreementProductionAssociationContacts(){
        ...
        for(Production_Associations__c productAssoc:[Select Id, Contact__c, Contact__r.Name From Production_Associations__c Where Production_Opp__c =:opportunityId AND Contact__c != NULL AND RecordType.DeveloperName = 'Production_Contacts' and Role__c includes ('Primary GT', 'CC: GT', 'Contract Signer')]) options.add(new SelectOption(productAssoc.Contact__c, productAssoc.Contact__r.Name));
        return options;
    }
    public List<SelectOption> getBidAgreementCreditCards(){
        List<SelectOption> options = new List<SelectOption>();
        options.add(new SelectOption('', '-- Select --'));
        for(Credit_Card__c creditCard : [Select Id, Contact__r.FirstName, Contact__r.LastName, Credit_card_number__c, Expiration_Date__c From Credit_Card__c Where Contact__c in (Select Contact__c From Production_Associations__c Where Production_Opp__c =:opportunityId AND Contact__c != NULL AND RecordType.DeveloperName = 'Production_Contacts' AND Status__c = 'Associated') AND Purpose__c includes ('All','Ground Transportation Only','Secondary')]){
            options.add(new SelectOption(creditCard.Id, (String.isNotBlank(creditCard.Contact__r.FirstName) ? creditCard.Contact__r.FirstName + ' ' : '') + creditCard.Contact__r.LastName + (creditCard.Credit_card_number__c != null ? ' ' + String.valueOf(creditCard.Credit_card_number__c) : '') + ', Exp:' + (String.isNotBlank(creditCard.Expiration_Date__c) ? creditCard.Expiration_Date__c : '')));
        }
        return options;
    }
    public void saveBidAgreement(){
        if(bidData != null && bidData.Id != null){
            try{
                ...
                update draft;
                draft = [SELECT Id, Name, Salutation__c, Attention__c, Primary_Contact_Id__c, Production_Assoc_Contact_Id__c, CC__c, Subject__C, Date__c, BCC__c, Body__c, Terms__c, CreatedBy.Name, CreatedDate, LastModifiedBy.Name, LastModifiedDate, Payment_Policy__c, Cancellation_Policy__c, Credit_Card_Id__c FROM Draft__c WHERE Id = :draft.Id];
                update bidData;
                /**
                *  @Conga: Update for Conga.
                */
                User emailFromUser = GTDashboardController.verifyEmailUser(UserInfo.getUserId(), opportunityid, bidData.GT__c);
                String emailBody = (String.isNotBlank(draft.Salutation__c) ? draft.Salutation__c + '<br/><br/>' : '') + (String.isNotBlank(draft.Body__c) ? draft.Body__c.removeStartIgnoreCase('<p>').removeEndIgnoreCase('</p>') : '') + (String.isNotBlank(emailFromUser.Name) ? '<br/>' + emailFromUser.Name : '') + '<br/>GT Coordinator';
                if(isCongaSignSent){
                    emailResult = sendEmail(bidAgreement_to, bidAgreement_cc, bidAgreement_bcc, draft.Subject__c, emailBody, null, opportunityId, bidData.GT__c); 
    ...
    public void openResendBid(){
        ...
        if(String.isNotEmpty(bidData.Vendor__r.ParentId)) for(Contact c:[SELECT Id, AccountId, Account.Name, Order__c, Account.ParentId, Name, Phone, Email, Fax, Designation_title__c FROM Contact WHERE AccountId =:bidData.Vendor__r.ParentId AND Email <> NULL ORDER BY Order__c ASC]){
            ...
            contactAuxList.add(new ContactWrapper(c, bidData.Vendor_Contact__c == c.Id, bidData.Vendor_Contact__c == c.Id));
            contactsByVendorParentBidMap.put(c.AccountId,contactAuxList.clone());
        }
        for(Contact c : [SELECT Id, AccountId, Account.Name, Order__c, Account.ParentId, Name, Phone, Email, Fax, Designation_title__c FROM Contact WHERE AccountId =:bidData.Vendor__c AND Email <> NULL ORDER BY Order__c ASC NULLS LAST]){
            if(String.isEmpty(bidData.Vendor_Contact__c) && c.Order__c == 1) bidData.Vendor_Contact__c = c.Id;
    ...
    public void changeContactVendorResendBid(){
        ...
        for(ContactWrapper cw : contactsByVendorBidMap.get(bidData.Vendor__c)){
            if(vendorcontact_actual_id == cw.c.Id) cw.selected = cw.disabled = true;
            else if(cw.selected && cw.disabled) cw.selected = cw.disabled = false;
        }
        if(bidData.Vendor__r.ParentId != null){
            for(ContactWrapper cw : contactsByVendorParentBidMap.get(bidData.Vendor__r.ParentId)){
                if(vendorcontact_actual_id == cw.c.Id) cw.selected = cw.disabled = true;
    ...
    public void sendReleaseByBid(){
        if(bidData != null && releaseVendorcontact_actual_id != null){
            ...
            mail.setToAddresses(toAddresses);
            if(ccAddresses.size() > 0) mail.setCcAddresses(new List<String>(ccAddresses));
            String dateIn = (bidData.GT__r.Start_Date__c != null ? Datetime.newInstance(bidData.GT__r.Start_Date__c.year(),bidData.GT__r.Start_Date__c.month(),bidData.GT__r.Start_Date__c.day()).format('dd MMMMM yyyy') : '');
            String dateOut = (bidData.GT__r.End_Date__c != null ? Datetime.newInstance(bidData.GT__r.End_Date__c.year(),bidData.GT__r.End_Date__c.month(),bidData.GT__r.End_Date__c.day()).format('dd MMMMM yyyy')  : '');
            mail.setSubject('Release Bid - ' + 
                            bidData.Production__r.Name + ' - ' + 
                            dateIn + '-' + dateOut
                           );
    ...
    public void changeContactVendorReleaseByBid(){
        ...
        for(ContactWrapper cw : contactsByVendor_releaseMap.get(bidData.Vendor__c)){
            if(releaseVendorcontact_actual_id == cw.c.Id) cw.selected = cw.disabled = true;
            else if(cw.selected && cw.disabled) cw.selected = cw.disabled = false;
        }
        if(bidData.Vendor__r.ParentId != null){
            for(ContactWrapper cw : contactsByVendorParent_releaseMap.get(bidData.Vendor__r.ParentId)){
    ...
    global static List<String> getDataPreviewResendBid(String opportunityid, String tripid, String bidid, String primarycontact,String url){
        ...
        User userActual = GTDashboardController.verifyEmailUser(UserInfo.getUserId(),opportunityid,tripid);      
        ...
        if(template.size() > 0) body = 'Dear ' + primary_name + ',<br/><br/>' + 
            template[0].HTMLValue.replace('{GroundTravelCoordinator}',userActual != null && userActual.Name != null ? userActual.Name : '')
            .replace('{CoordinatorPhone}',userActual != null && userActual.Phone != null ? userActual.Phone : '')
            .replace('{VendorURL}','<a href="https://ccdev-roadrebel.cs42.force.com/" target="_blank">' + bidAux[0].Vendor__r.Name + '</a>')
    ...
    public static String sendEmail(String toList, String ccList, String bccList, String subject, String body, String attachs, String oppid, String tripid){
        try{
            ...
            User userActual = GTDashboardController.verifyEmailUser(UserInfo.getUserId(),oppid,tripid);
            ...
            System.debug('###GTDashboard_Tab_GT_Bid_Controller.sendEmail() - Email Exception: ' + e.getMessage());
    ...
    public void calculateTotalPrice(){
        if(bidRevenue != null && bidData.Production__r.Office_Location__c != 'RRE') bidRevenue.Total_Price__c = (bidRevenue.Base_Cost__c != null ? bidRevenue.Base_Cost__c : 0) + (bidRevenue.Commission__c != null ? bidRevenue.Commission__c : 0) + (bidRevenue.Gratuity__c != null ? bidRevenue.Gratuity__c : 0);
    }
    public void calculateRevenue(){
        if(bidRevenue != null && bidData.Production__r.Office_Location__c != 'RRE'){
            if(bidRevenue.Commission__c != null) bidRevenue.Rate__c = (100 * bidRevenue.Commission__c)/(bidRevenue.Base_Cost__c != null ? bidRevenue.Base_Cost__c : 0);
            else if(bidRevenue.Rate__c != null) bidRevenue.Commission__c = (bidRevenue.Rate__c * (bidRevenue.Base_Cost__c != null ? bidRevenue.Base_Cost__c : 0))/100;
            calculateTotalPrice();
        }
    }
    public void calculateRevenueByCommission(){
        if(bidRevenue != null && bidRevenue.Commission__c != null && bidData.Production__r.Office_Location__c != 'RRE'){
    ...
    public void calculateRevenueByRate(){
        if(bidRevenue != null && bidRevenue.Rate__c != null && bidData.Production__r.Office_Location__c != 'RRE'){
    ...
    public void getBidRevenue(){
        List<Revenue__c> bidRevenues = [Select id, Base_Cost__c, Commission__c, Rate__c, VAT_Rate__c, Gratuity__c, Total_Price__c, VAT_Included_in_Total__c, CurrencyIsoCode from Revenue__c where Bid__c =:bidDataMap.get(tripBidNumber).b.Id and RecordType.DeveloperName = 'Ground_Rate_Travel_Commission' order by createdDate desc limit 1];
        bidRevenue = !bidRevenues.isEmpty() ? bidRevenues[0] : new Revenue__c(RecordTypeId = Schema.SObjectType.Revenue__c.getRecordTypeInfosByName().get('Ground Rate & Travel Commission') != null ? Schema.SObjectType.Revenue__c.getRecordTypeInfosByName().get('Ground Rate & Travel Commission').getRecordTypeId() : null, Bid__c = bidDataMap.get(tripBidNumber).b.Id, CurrencyIsoCode = 'USD');
    ...
    public void getSymbol(){
        for(SelectOption so : GTDashboardController.PickThePicklist('Revenue__c','CurrencyIsoCode')) symbolMap.put(so.getValue(),'');
    ...
    public void congaResendBid(){
        if(String.isNotBlank(resendBidAgreement_contactPrimary)){
            ...
            for(ContactWrapper contactWrapperItem:(bidData.Vendor__c != null ? contactsByVendorBidMap.get(bidData.Vendor__c) : new List<ContactWrapper>())){
                ...
                if(contactWrapperItem.disabled && contactWrapperItem.selected) primaryContact = contactWrapperItem.c;
            }
            ...
            congaParameters.add('&id=' + bidData.id + (congaQueriesMap.containsKey('GT - BidById') ? '&QueryId=[Bid]' + congaQueriesMap.get('GT - BidById').Id + '?pv0=' + bidData.id + (congaQueriesMap.containsKey('GT - SubTripsByBidId') ? ',[SubTrips]' + congaQueriesMap.get('GT - SubTripsByBidId').Id + '?pv0=' + bidData.id : '') + (congaQueriesMap.containsKey('GT - ContactById') ? ',[Contact]' + congaQueriesMap.get('GT - ContactById').Id + '?pv0=' + resendBidAgreement_contactPrimary : '') + (congaQueriesMap.containsKey('GT - UserById') ? ',[EmailFromUser]' + congaQueriesMap.get('GT - UserById').Id + '?pv0=' + (String.isNotBlank(resendBidAgreement_emailFromUserId) ? resendBidAgreement_emailFromUserId : '') : '') : '') + '&CongaEmailTemplateId='+ resendBidAgreement_congaEmailTemplateId + '&EmailSubject=' + resendBidAgreement_congaEmailSubject.replace(' ', '+') + (String.isNotBlank(resendBidAgreement_congaEmailFromId) ? '&EmailFromID=' + resendBidAgreement_congaEmailFromId : '') + '&EmailToId=' + resendBidAgreement_contactPrimary + '&EmailCC=' + ccEmails + '&TemplateId=' + resendBidAgreement_congaTemplateId + '&OFN=GT+Bid-' + bidData.Name);
            try{
                ...
                if(bidData.GT__r.Status_Ground__c != 'Contracted'){
                    update new Ground__c(id = bidData.GT__c, Status_Ground__c = 'Bid Request Stage');
                    List<Itinerary__c> itineraries =  [SELECT Id FROM Itinerary__c WHERE GT__c =:bidData.GT__c AND RecordType.DeveloperName = 'GT_Itinerary'];
                    if(!itineraries.isEmpty()) update new Itinerary__c(Id = itineraries[0].Id, Status__c = 'Bid Request Stage');
                }
                ...
                System.debug('###GTDashboard_Tab_GT_Bid_Controller.congaResendBid() - Exception: ' + e.getMessage());
            }
        }
    }
    @future(callout=true)
    public static void congaSendEmail(List<String> congaParameters){
        ...
        for(String parameters:congaParameters){
            ...
            try{
                ...
                System.debug('###GTDashboard_Tab_GT_Bid_Controller.congaEmail(parameters = ' + parameters + ') => Response Status Code: ' + response.getStatusCode() + ', Response Body: ' + (String.isNotEmpty(response.getBody()) ? response.getBody().left(255) : ''));
            }
            catch(Exception e){
                System.debug('###GTDashboard_Tab_GT_Bid_Controller.congaEmail() - Callout Exception: ' + e.getMessage());
            }
        }
    }
    /**
